<template>
  <div class="mb-4 player-turn-order"
    v-if="isNaN(playerTurnOrderFirstVisible) || (playerTurnOrderFirstVisible <= currentCardIndex)">
    <div class="row">
      <div class="col-sm">
        <div class="card d-flex extension-card">
          <div class="card-body">
            ...
            <div v-if="playerTurnOrder.players">
              <div v-for="(player, index) in playerTurnOrder.players" v-bind:key="index">
                <div class="justify-content-between d-flex">
                  <span></span>
                  <span class="my-auto"
                    v-bind:class="{ 'font-weight-bold': (index == (currentCardIndex + playerTurnOrder.activePlayerOffset) % playerTurnOrder.players.length) }"
                    style="cursor:pointer" v-on:click="makeActivePlayer(index)" v-if="index != currentEditIndex">
                    <span
                      v-if="index == (currentCardIndex + playerTurnOrder.activePlayerOffset) % playerTurnOrder.players.length">
                      –</span>
                    {{ player }}
                    <span
                      v-if="index == (currentCardIndex + playerTurnOrder.activePlayerOffset) % playerTurnOrder.players.length">–
                  ...
                  <div>
                    <button class="btn btn-sm btn-outline-dark m-1 px-1 edit-button" v-on:click="editPlayer(index)"
                      v-if="currentEditIndex !== index">
                      <IBiPencil />
                    </button>
                    <div v-else>
                      <button class="btn btn-sm btn-outline-dark m-1 px-1" v-on:click="deletePlayer(index)">
                        <IBiTrash />
                      </button>
            ...
            <button :disabled="!newPlayer" class="btn btn-outline-dark m-2" v-on:click="addPlayer(newPlayer)"
              v-dompurify-html="playerTurnOrderButtonLabel" v-if="playerTurnOrderButtonLabel != undefined">Add
              Player</button>
            <button v-else :disabled="!newPlayer" class="btn btn-outline-dark m-2" v-on:click="addPlayer(newPlayer)">Add
              Player</button>
