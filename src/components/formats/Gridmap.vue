<template>
  <div class="gridmap game-room container" v-if="roomInfo" v-bind:class="['style-template-' + customOptions.styleTemplate]">
    ...
    <div class="mb-4">
        ...
        <div class="mt-4 gridmap-main" v-if="(dataReady && firebaseReady) && !error">
          ...
          <div class="gridmap-map row mt-3" align="center">
            <div class="col">
              <div class="row gridmap-row" v-for="(row, rowIndex) in parseInt(customOptions.mapRows)" v-bind:key="rowIndex">
                <div v-for="(col, colIndex) in parseInt(customOptions.mapColumns)" v-bind:key="colIndex" class="col gridmap-col">
                  <div class="my-4" style="white-space: pre-line">
                    
                    <div v-html="originalGridmapData[roomInfo.currentMapData[rowIndex*parseInt(customOptions.mapColumns) + colIndex]]" class="map-element mb-2"></div>
    ...
    this.$bind('roomInfo', roomsCollection.doc(this.roomID))
      ...
      .then(() => {
        if (!this.roomInfo){
          ...
          roomsCollection.doc(this.roomID).set({extensionData: this.tempExtensionData, currentMapData:[0,1,2]})
              ...
              for (let j = 0; j < rowLayout.length; j++) {
                ...
                switch (rowLayout[j]) {
                  ...
                  case "4":
                    ...
                    break;
                  default:
                    bootstrapLayout.push('col-sm-12 gridmap-col ' + rowClass)
                 }
               }
            ...
            if (item.values[0].formattedValue !== "option" && item.values[0].formattedValue !== "extension"){
              ...
              if (item.values[0].formattedValue >= 0){
                ...
                if (item.values[0].formattedValue == 1){
                  this.originalGridmapData.push(item.values[3].formattedValue)
                }
              }
            }
