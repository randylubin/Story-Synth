    ...
    >
      ...
      >
        <b-button
          ...
          :disabled="roomInfo.xCardIsActive || roomInfo.popCardIsActive"
      ...
      >
        ...
        >
          ...
          <b-alert
            show
            class="mx-3"
            v-dompurify-html="customOptions.lastCardReminderText"
            variant="info"
            v-if="
          ...
          <div
            class="card-body align-items-center justify-content-center"
            v-if="roomInfo.popCardIsActive && !roomInfo.xCardIsActive"
            v-bind:class="{
              'card-body': !customOptions.cardBackgroundImage,
              'card-img-overlay':
                customOptions.cardBackgroundImage &&
          ...
          >
            <div class="mt-5 pt-5 mb-5">
              <div
                class="pop-card-text"
                v-dompurify-html="roomInfo.popCardText"
              ></div>
            </div>
            <button class="btn btn-outline-dark mt-5" v-on:click="hidePopCard">
              Continue
            </button>
          </div>
...
export default {
  ...
  methods: {
    ...
    setPopCard(cardText) {
      this.$emit("firebase-update", {
        popCardText: cardText,
        popCardIsActive: true,
      });
    },
    hidePopCard() {
      this.$emit("firebase-update", {
        popCardIsActive: false,
      });
    },
    xCard() {
      this.$emit("firebase-update", {
    ...
    setupPopCards() {
      this.popcards = this.parsePopCardsFromCustomOptions();
      console.log("pops", this.popcards);
      if (this.popcards.length > 0) {
        this.shouldRenderPopCards = true;
      }
    },
    parseFromOptions(regex) {
    ...
    parsePopCardsFromCustomOptions() {
      const regex = /^popcard-(\d)-(\w+)$/;
      return this.parseFromOptions(regex);
    },
